...
pub const ENVIRONMENT_CONTRACT_VERSION: &str = "environment-v1";
...
pub fn evaluate_environment_readiness(
    primary_command: &str,
    detector_keys: &[String],
    module_keys: &[String],
    renderer_keys: &[String],
    ...
    let checks = vec![
        ...
        ReadinessCheck {
            key: "detector-registry",
            passed: ["os", "cpu", "memory", "disk", "shell", "terminal"]
                .iter()
                .all(|required| detector_keys.iter().any(|key| key == required)),
            detail: format!("registered detectors: {}", detector_keys.join(", ")),
        },
...
mod tests {
    use super::{
        BASELINE_CONTRACT_VERSION, ENVIRONMENT_CONTRACT_VERSION, FOUNDATION_CONTRACT_VERSION,
    ...
    #[test]
    fn readiness_passes_for_environment_happy_path() {
        let report = evaluate_environment_readiness(
            "corefetch",
            &[
                "os".to_owned(),
                "cpu".to_owned(),
        ...
        assert_eq!(report.contract_version, ENVIRONMENT_CONTRACT_VERSION);
        assert!(report.ready);
        assert!(report.checks.iter().all(|check| check.passed));
    }

    #[test]
    fn readiness_fails_for_environment_when_terminal_flow_is_missing() {
        let report = evaluate_environment_readiness(
            "corefetch",
            &[
                "os".to_owned(),
